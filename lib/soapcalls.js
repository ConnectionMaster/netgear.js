...
exports.action = {
	...
	// was commented out (and its request body wrongly copy-pasted from getWANIPConnection's
	// GetInfo call) since the original v4.4.3 - fixed per a real R8000 capture confirming the
	// correct action, see github.com/MatMaul/pynetgear#20
	getWANDNSLookUpStatus: 'urn:NETGEAR-ROUTER:service:WANIPConnection:1#GetDNSLookUpStatus',
	...
	// confirmed against a real R8000 capture, see github.com/MatMaul/pynetgear#20
	getAllMACAddresses: 'urn:NETGEAR-ROUTER:service:ParentalControl:1#GetAllMACAddresses',
	...
	getAllSatellites: 'urn:NETGEAR-ROUTER:service:DeviceInfo:1#GetAllSatellites', // ***NEW*** Orbi mesh satellites, see pynetgear
	...
	// confirmed against a real R8000 capture, see github.com/MatMaul/pynetgear#20
	getWPASecurityKeys: 'urn:NETGEAR-ROUTER:service:WLANConfiguration:1#GetWPASecurityKeys',
	get5GWPASecurityKeys: 'urn:NETGEAR-ROUTER:service:WLANConfiguration:1#Get5GWPASecurityKeys',
	get5G1WPASecurityKeys: 'urn:NETGEAR-ROUTER:service:WLANConfiguration:1#Get5G1WPASecurityKeys',
...
// Orbi mesh satellite list, see https://github.com/MatMaul/pynetgear
...
exports.getWANDNSLookUpStatus = (sessionId) => {
	// NOTE: was wrongly copy-pasted as a duplicate of getWANIPConnection's GetInfo call (same
	// body verbatim) since the original v4.4.3, meaning this never actually invoked
	// GetDNSLookUpStatus at all. Fixed per a real R8000 capture, see
	// github.com/MatMaul/pynetgear#20 - that capture also shows the router mislabels the
	// response as GetConnectionTypeInfoResponse instead of GetDNSLookUpStatusResponse, a
	// separate firmware bug handled on the parsing side in netgear.js#getWANDNSLookUpStatus.
	const soapBody = `<v:Body>
		<n0:GetDNSLookUpStatus xmlns:n0="urn:NETGEAR-ROUTER:service:WANIPConnection:1" />
	</v:Body>`;
	return soapEnvelope(sessionId, soapBody);
};
...
// see https://github.com/MatMaul/pynetgear
exports.getAllMACAddresses = (sessionId) => {
	const soapBody = `<v:Body>
		<M1:GetAllMACAddresses xmlns:M1="urn:NETGEAR-ROUTER:service:ParentalControl:1" />
	</v:Body>`;
	return soapEnvelope(sessionId, soapBody);
};
...
// confirmed against a real R8000 capture, see github.com/MatMaul/pynetgear#20
exports.getWPASecurityKeys = (sessionId) => {
	const soapBody = `<v:Body>
...
exports.get5GWPASecurityKeys = (sessionId) => {
	const soapBody = `<v:Body>
		<M1:Get5GWPASecurityKeys xmlns:M1="urn:NETGEAR-ROUTER:service:WLANConfiguration:1" />
	</v:Body>`;
	return soapEnvelope(sessionId, soapBody);
};

exports.get5G1WPASecurityKeys = (sessionId) => {
	const soapBody = `<v:Body>
		<M1:Get5G1WPASecurityKeys xmlns:M1="urn:NETGEAR-ROUTER:service:WLANConfiguration:1" />
	</v:Body>`;
	return soapEnvelope(sessionId, soapBody);
};
...
exports.setUserOptionsTC = (sessionId, enabled) => {
	const soapBody = `<v:Body>
		<n0:SetUserOptionsTC xmlns:n0="urn:NETGEAR-ROUTER:service:UserOptionsTC:1">
