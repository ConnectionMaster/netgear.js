...
const flattenEntries = (entries, stripNewPrefix = false) => {
	...
	Object.keys(entries).forEach((key) => {
		...
		// tag, which the original code surfaced as `undefined` (see e.g. the `info` typedef's
		// documented `VPNVersion: undefined`); fast-xml-parser instead yields `''` - normalize back
		// to `undefined` so callers see the exact same shape as before.
		result[propName] = value === '' ? undefined : value;
	});
	return result;
};
